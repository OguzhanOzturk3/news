...
const Profile = (props) => {
  ...
  const fetchNews = async () => {
    const response = await axios.get(`https://newsapi.org/v2/top-headlines?country=tr&q=${searched}&apiKey=fec3069e798949948daf33b7d13c482f`);
  ...
  const prevSlide = () => {
    ...
    if (slides.length === 1) {
      activeSlide.classList.add('active')
        ...
        slides[parseInt(slides.length) - 1].classList.add('active');
      }
    }
  ...
  return (
    <div>
      <h1 className='page-title'>Searched Results for '{searched}'</h1>
        ...
        <div>
          ...
          {slider.map((slide_item) => (
            <div className='slide'>
              ...
              <p className='news-content'>{slide_item.content}</p>
            </div>
